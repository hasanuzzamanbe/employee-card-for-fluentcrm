...
$employee = (new EmployeeCard\Classes\ContactHelper())->getByHash(get_query_var('person'));
$designation = '';
$socialInfo = [];
$iconClasses = [
...
$customValues = Arr::get($employee, 'custom_values', []);

if (!empty($customValues)) {
    $designation = Arr::get($customValues, 'designation', '');

    $socialSites = array_keys($iconClasses);

    $socialInfo = Arr::only($customValues, $socialSites);
};
...
<head>
    ...
    <?php if (!is_null($employee)): ?>
        ...
        <title>Contact <?= esc_html($employee->name); ?></title>
    <?php else: ?>
...
<?php if (!is_null($employee)): ?>
    ...
    <div class="flex justify-center ">
        <div class="w-[300px] lg:w-[400px] shadow-lg relative h-screen employee-card-wrapper">
            <div class="pb-[50px]">
                <div class="w-full relative">
                    <img id="employee_profile_image" class="w-full object-fill" src="<?= esc_url($employee->photo); ?>"/>
                    <div class="absolute bottom-0 w-full bg-gradient-to-t from-black min-h-24 p-4">
                        <h1 class="text-center text-white m-2" id="employee_name"><?= esc_html($employee->full_name); ?></h1>
                        <p id="employee_designation"
                           class="text-center text-white m-0"><?= esc_html($designation); ?></p>
                        <div class="flex gap-4 mt-2">
                            <?php if ($employee->phone): ?>
                            <div class="w-10 h-10 rounded-full bg-gray-400 flex justify-center items-center cursor-pointer">
                                <a href=tel:<?= esc_attr($employee->phone); ?>">
                            ...
                            <div class="w-10 h-10 rounded-full bg-gray-400 flex justify-center items-center cursor-pointer">
                                <a href="mailto:<?= esc_attr($employee->email); ?>">
...
<!--                            </div>-->
                    ...
                    <div class="mt-2 px-4 py-2">
                        <div class="rounded-lg shadow bg-white p-8 grid grid-cols-7">
                            <div class="col-span-1">
                                <i class="<?= esc_attr($iconClasses['phone']); ?>"></i>
                            </div>
                            <div id="employee_phone" class="col-span-6">
                                <a href=tel:<?php echo esc_attr($employee->phone); ?>">
                                    <?= esc_html($employee->phone); ?>
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-2 px-4 py-2">
                        <div class="rounded-lg shadow bg-white p-8 grid grid-cols-7">
                            ...
                            <div id="employee_email" class="col-span-6">
                                <a href="mailto:<?= $employee->email ? esc_html($employee->email) : ''; ?>">
                                    <?= esc_html($employee->email)?>
                                </a>
                            </div>
                        </div>
                    </div>
                ...
                <?php if ($employee->address_line_1) : ?>
                <div class="mt-2 px-4 py-2">
                    <div class="rounded-lg shadow bg-white p-8 grid grid-cols-7">
                        <div class="col-span-1">
                        ...
                        <?php foreach ($socialInfo as $name => $info):
                            if (!empty($info)):
                                ...
                                <div class="grid grid-cols-7">
                                    <div class="col-span-1 flex items-center">
                                        <div class="w-8 h-8 flex justify-center items-center">
                                            <?php if (isset($iconClasses[$name])): ?>
                                                <i class="<?= esc_attr($iconClasses[$name]); ?>"></i>
                                            <?php else: ?>
                                    ...
                                    <div class="col-span-6">
                                        <p class="my-2"><a href="<?= esc_url($info) ?>"><?= esc_html($name) ?>  </a></p>
                                    </div>
                                </div>
            ...
            <div class="px-4 fixed bottom-0 w-[300px] lg:w-[400px]">
...
<?php if (!is_null($employee)): ?>
    <script src="<?= esc_url(EMPLOYEE_CARD_URL . 'assets/admin/js/front-end.js'); ?>"></script>
<?php endif; ?>
</body>
</html>
